---
description: Show the available agent team (Core Squad + Specialists) and commands, with when to use each.
---

Show me the available specialists and commands, briefly, and ask what I'm working on to suggest who to delegate to.
...
## Commands
- **/council** — advisory panel (CEO/CFO/CTO/… + Red Team + Chairman) for strategic decisions.
- **/preflight** — parallel pre-merge check (qa + code-reviewer + security + language specialist).
- **/retro** — retrospective: extract learnings and decide where to store them.
- **/handoff** — clean handoff of the current state to resume later.
- **/team** — this help.
